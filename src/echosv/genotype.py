...
def genotype_main(params=None):
    import argparse
    parser = argparse.ArgumentParser(prog="genotype", description="Annotate SVs from given dataset.")
    ...
    if args.longread:
        anno = AnnSV_lrs(sv_vcf=args.input, bam_list=args.bam, out_file=args.output, min_mapq=args.min_mapq, dist_threshold=args.dist_threshold, ratio_of_seqsize=args.ratio_of_seqsize, verbose=args.verbose)
    elif args.shortread:
        anno = AnnSV_srs(sv_vcf=args.input, bam_list=args.bam, out_file=args.output, min_mapq=args.min_mapq, dist_threshold=args.dist_threshold, ratio_of_seqsize=args.ratio_of_seqsize, verbose=args.verbose)
    else:
        raise ValueError("Please specify either --longread or --shortread option.")
    anno.run()
    print(f"Genotyping finished in {timeit.default_timer() - start_time:.2f} seconds")
