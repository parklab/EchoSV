...
def match_main(params=None):
    ...
    parser.add_argument("-i", "--input", help="Input config file, see an example `test_colo829_config.json` in the test_data directory.", required=True)
    ...
    if config.get("chains"):
        ...
        sv_cmp_liftover(config=config, verbose=args.verbose)
